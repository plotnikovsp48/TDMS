...
{ 
  ...
  if (this.level>0) 
    if (this.isLastNode) //the last child in the children array 
	    leftSide = leftSide + "<td valign=top>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 style='margin-top:-2px; margin-bottom:-2px' border=0></a></td>"
    else 
      leftSide = leftSide + "<td valign=top background=" + ICONPATH + "ftv2vertline.gif>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 style='margin-top:-2px; margin-bottom:-2px' border=0></a></td>"
  ...
  if (USEICONS)
  {
    ...
    docW = docW + "<img id='folderIcon" + this.id + "' name='folderIcon" + this.id + "' src='" + this.iconImageSrc() + "' border=0 style='margin-right:5px;'></a>"
  }
  else
  {
	  if (this.prependHTML == "")
...
{ 
  var leftSide = leftSideHTML(this.leftSideCoded)
  var docW = ""
  var fullLink = "href=\""+this.link+"\" target=\""+this.target+"\" onClick=\"clickOnLink('"+this.getID()+"');\"";
  this.isRendered = 1
  if (this.level>0) 
    if (this.isLastNode) //the last 'brother' in the children array 
    { 
      leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2lastnode.gif' width=16 height=22 style='margin-top:-2px; margin-bottom:-2px'></td>"
    } 
    else 
    { 
      leftSide = leftSide + "<td valign=top background=" + ICONPATH + "ftv2vertline.gif><img src='" + ICONPATH + "ftv2node.gif' width=16 height=22 style='margin-top:-2px; margin-bottom:-2px'></td>"
    } 
  ...
  docW = docW + "<tr>" + leftSide + "<td valign=top>"
  if (USEICONS)
      docW = docW + "<a " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<img id='itemIcon"+this.id+"' " + "src='"+this.iconSrc+"' border=0 style='margin-right:5px;'>" + "</a>"
  else
	  if (this.prependHTML == "")
...
function leftSideHTML(leftSideCoded) {
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=top background=" + ICONPATH + "ftv2vertline.gif><img src='" + ICONPATH + "ftv2vertline.gif' width=16 height=22 style='margin-top:-2px; margin-bottom:-2px'></td>"
		}
		if (leftSideCoded.charAt(i) == "0")
		{
			retStr = retStr + "<td valign=top><img src='" + ICONPATH + "ftv2blank.gif' width=16 height=22 style='margin-top:-2px; margin-bottom:-2px'></td>"
		}
	}
	return retStr
}
...
function loadFrames(page1, page2, page3) {
	window.parent.frames['bottom'].location=page1
	window.parent.frames['rightfrm'].frames['basefrm'].location=page3
	parent.frames['rightfrm'].document.all.rightframe.rows='80%,*';
	window.parent.frames['rightfrm'].frames['filesfrm'].location=page2;
}
...
function loadSynchPage(linkID) {
    ...
    parent.treeframe.highlightObjLink(parent.treeframe.findObj(linkID));
    var a = docObj.link.split("'");
param1 = "..\/..\/" +a[1];
param2 = "..\/..\/" +a[3];
param3 = "..\/..\/" +a[5];
    loadFrames(param1,param2,param3);
    if (typeof parent.treeframe.document.body != "undefined") 
     parent.treeframe.document.body.scrollTop=docObj.navObj.offsetTop
} 

function loadRefPage(linkID) {
	docObj = parent.treeframe.findObj(linkID);
	var a = docObj.link.split("'");
//	param1 = "..\/..\/" +a[1];
	param1 = a[1];
...
if ((brouzer == 'Windows') && (browser == 'IE')) {
	var ssilka = window.open(url, "", "width=500 scrollbars=no");
}else{
if (brouzer == 'Windows') {
if (browser == 'Netscape') {
var decalLarger = -12;
...
var largerEkran = 500;
var hEkran = 200;
var ssilka = window.open(url,"","width=" + largerEkran,"scrollbars=no");
ssilka.moveTo(0 - decalLarger,0 - decalH);
}
}
else {
if (brouzer == 'Macintosh') {
if (browser == 'Netscape') {
var decalLarger = -10;
var decalH = -30;
var largerEkran = 500;
...
var largerEkran = 500;
var hEkran = 200;
var ssilka = window.open(url,"", "scrollbars=no");
ssilka.moveTo(0,0);
ssilka.resizeTo(largerEkran);
}
}
}
}
